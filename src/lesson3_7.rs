...
use glam::{UVec2, Vec2, Vec3, Vec3Swizzles, Vec4, Vec4Swizzles};
...
fn render_(image_size: u32, frame: &mut Frame) {
    ...
    for tri in model() {
        ...
        if luma > 0.0 {
            ...
            for_coord_in_triangle(verts, |pos, bar| {
                let pix = frame.get_mut(pos.x, pos.y);
                let z: f32 = bary_interp(bar, zs.to_array());
                let uv: Vec2 = bary_interp(bar, uvs);
                let uv = texsize - uv * texsize;
...
fn for_coord_in_triangle(verts: [UVec2; 3], mut cb: impl FnMut(UVec2, Vec3)) {
...
fn texture() -> image::Rgb32FImage {
    image::load_from_memory_with_format(
        include_bytes!("./obj/african_head/african_head_diffuse.png"),
        ImageFormat::Png,
    )
    .unwrap()
    .to_rgb32f()
