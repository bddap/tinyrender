...
impl Shader for Ssao {
    ...
    fn fragment(&self, z: Self::Varying, _: [Self::Intermediate; 3]) -> Option<Vec3> {
        Some(Vec3::splat(z))
    }
}

fn ssao_post_process(inp: &mut Frame, out: &mut Frame) {
    ...
    for p in inp.pix.iter_mut() {
        p.w = remap(wmin, wmax, 0.0, 1.0, p.w).max(-1.0);
    }

    for x in 0..inp.width() {
        for y in 0..inp.height() {
            if inp.try_get(x as u32, y as u32).unwrap().w < 0.0 {
                continue;
            }
            let sum: f32 = [
                [-1, -1],
            ...
            let total = sum / ((PI / 2.0) * 8.0);

            *out.get_mut(x as u32, y as u32) = Vec4::splat(total.powf(100.0));
        }
    }
}
...
fn max_elevation_angle(fr: &Frame, start: IVec2, direction: IVec2) -> f32 {
    ...
    let mut pos = start;
    let w_at_start = fr.try_get(start.x as u32, start.y as u32).unwrap().w;
    ...
    for i in 1..1000 {
        ...
        let w_diff = fr.try_get(pos.x as u32, pos.y as u32).unwrap().w - w_at_start;
        let distance = i as f32 * direction_len;
        max_angle = max_angle.max((w_diff / distance).atan());
        pos += direction;
    }
    max_angle
}
...
fn render_(image_size: u32, frame: &mut Frame) {
    ...
    let viewport = Mat4::from_translation(Vec3::new(halfscreen, halfscreen, 0.0))
    ...
    let mut frameclone = frame.clone();
    ssao_post_process(&mut frameclone, frame);
}
...
fn remap(lowin: f32, highin: f32, lowout: f32, highout: f32, x: f32) -> f32 {
    (x - lowin) / (highin - lowin) * (highout - lowout) + lowout
}

#[test]
fn t_remap() {
    assert_eq!(remap(0.0, 1.0, 0.0, 1.0, 0.5), 0.5);
    assert_eq!(remap(0.0, 1.0, 0.0, 2.0, 0.5), 1.0);
    assert_eq!(remap(0.5, 1.0, 0.0, 2.0, 0.5), 0.0);
    assert_eq!(remap(0.0, 10.0, 1.0, 2.0, 1.0), 1.1);
}
