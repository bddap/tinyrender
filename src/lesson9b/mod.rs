...
async fn render_async(img: &mut RgbaImage) {
    ...
    let rand_instances: Vec<InstanceInput> = (0..5_000_000) // five million!
        .map(|_| {
            ...
            let m: f32 = 1.0 / (x * x + y * y + z * z).sqrt();
            ...
            InstanceInput {
                loc: [x, y, z],
                scale: lerp(0.04, 0.06, rand::random::<f32>()),
                color: [
                    lerp(0.13, 0.17, rand::random::<f32>()) + 0.1,
                    lerp(0.08, 0.12, rand::random::<f32>()) + 0.1,
                    lerp(0.03, 0.07, rand::random::<f32>()) + 0.1,
                ],
            }
        })
        .chain([
            InstanceInput {
                // not sure why yet, but a negative z value stops the object from being drawn
                // draw is also blocked if z > 1
                loc: [0.3, 0.0, 0.5],
                scale: 0.3,
                color: [0.15, 0.1, 0.05],
            },
    ...
    pipeline.upload_data(&rand_instances);
